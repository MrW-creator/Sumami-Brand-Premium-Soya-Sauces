...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onClose, onSettingsUpdated, onAddTestProduct }) => {
  ...
  // --- HELPER FOR KEY VALIDATION (NOW LOOKING FOR SECRET KEYS) ---
  const isValidSecretKey = (key: string) => key.trim().startsWith('sk_');
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-gray-100 overflow-auto no-print">
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl no-print">
        ...
        {/* --- SETTINGS TAB (UPDATED FOR SECRET KEYS) --- */}
        {viewTab === 'settings' && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 max-w-2xl mx-auto">
                 <div className="flex items-center gap-4 mb-6 border-b pb-4">
                     ...
                     <div>
                         ...
                         <p className="text-gray-500">Manage Yoco API Keys & Redirect Checkout.</p>
                     </div>
                 </div>
                 ...
                 <div className="space-y-6">
                     ...
                     <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 text-sm text-amber-900 flex items-start gap-3">
                        ...
                        <div>
                           <strong>Switching to Secure Redirect Checkout</strong>
                           <p className="mt-1 mb-2 opacity-90">
                              To prevent popup blockers and visual glitches, we now use the Yoco Redirect Page (like WooCommerce).
                              This method requires your <strong>Secret Key (sk_)</strong>, which will be stored securely in the Database.
                           </p>
                           <p className="text-xs font-bold uppercase tracking-wide opacity-75">
                              Find it: Yoco Portal &gt; Sell Online &gt; Payment Gateway &gt; Keys &gt; "Secret Key" section.
                           </p>
                        </div>
                     </div>
                     ...
                     <div>
                         <label className="block text-sm font-bold text-gray-700 mb-1">
                            Yoco Secret Key (Test Mode)
                            <span className="block text-xs font-normal text-gray-400">Must start with <code className="bg-gray-100 px-1 rounded">sk_test_</code></span>
                         </label>
                         <div className="relative">
                            <input 
                                type={showTestKey ? "text" : "password"}
                                className={`w-full border rounded-lg p-3 pr-12 font-mono text-sm outline-none transition-all ${isPublicKey(settings.yoco_test_key) ? 'border-red-500 bg-red-50 text-red-900' : 'border-gray-300 bg-gray-50 focus:border-amber-500 focus:bg-white'}`}
                         ...
                         {isPublicKey(settings.yoco_test_key) && <p className="text-xs text-red-600 font-bold mt-1 flex items-center gap-1"><AlertTriangle className="w-3 h-3"/> Error: You entered a Public Key (pk_). Please use the Secret Key (sk_).</p>}
                     </div>
                     
                     <div>
                         <label className="block text-sm font-bold text-gray-700 mb-1">
                            Yoco Secret Key (Live Mode)
                            <span className="block text-xs font-normal text-gray-400">Must start with <code className="bg-gray-100 px-1 rounded">sk_live_</code></span>
                         </label>
                         <div className="relative">
                            <input 
                                type={showLiveKey ? "text" : "password"}
                                className={`w-full border rounded-lg p-3 pr-12 font-mono text-sm outline-none transition-all ${isPublicKey(settings.yoco_live_key) ? 'border-red-500 bg-red-50 text-red-900' : 'border-gray-300 bg-gray-50 focus:border-amber-500 focus:bg-white'}`}
                         ...
                         {isPublicKey(settings.yoco_live_key) && <p className="text-xs text-red-600 font-bold mt-1 flex items-center gap-1"><AlertTriangle className="w-3 h-3"/> Error: You entered a Public Key (pk_). Please use the Secret Key (sk_).</p>}
                     </div>
                 ...
                 <div className="p-8 overflow-y-auto">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                             ...
                             <p className="text-xl font-bold">R {selectedOrder.total_amount}</p>
                        </div>
                    </div>
                 </div>
             </div>
